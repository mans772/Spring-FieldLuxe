...
public class PageCreator {
	...
	private int productTotalCount; //게시판의 총 게시물 수
	...
	private void calcDataOfPage() {
		...
		next = (productTotalCount <= (endPage * paging.getCountPerPage())) ? false : true;
		
		//다음 버튼 비활성화라면 끝 페이지 재보정 하기
		if(!isNext()) {
			endPage = (int) Math.ceil(productTotalCount / (double) paging.getCountPerPage());
		}
